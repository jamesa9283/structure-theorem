...
import tactic.group group_theory.subgroup
...
(exists_generator [] : ∃ g : α, ∀ x, x ∈ add_subgroup.gmultiples g)

attribute [to_additive is_add_cyclic] is_cyclic
...
attribute [to_additive is_add_cyclic_of_subsingleton] is_cyclic_of_subsingleton

/-
@[priority 100]
instance is_add_cyclic_of_subsingleton [add_group α] [subsingleton α] : is_add_cyclic α :=
⟨⟨0, λ x, by { rw subsingleton.elim x 0, exact add_subgroup.mem_gmultiples 0 }⟩⟩
-/
...
/-- A addititive cyclic group is always commutative. This is not an `instance` because often we have a better
proof of `comm_group`. -/
def is_cyclic.add_comm_group [hg : add_group α] [is_add_cyclic α] : add_comm_group α :=
{ add_comm := λ x y, show x + y = y + x,
    from let ⟨g, hg⟩ := is_add_cyclic.exists_generator α in
    let ⟨n, hn⟩ := hg x in let ⟨m, hm⟩ := hg y in
    hm ▸ hn ▸ gsmul_add_comm _ _ _,
  ..hg }
...
lemma monoid_hom.map_add_cyclic {G : Type*} [add_group G] [h : is_add_cyclic G] (σ : G →+ G) :
  ∃ m : ℤ, ∀ g : G, σ g = m • g :=
begin
  obtain ⟨h, hG⟩ := is_add_cyclic.exists_generator G,
  obtain ⟨m, hm⟩ := hG (σ h),
  use m,
  intro g,
  obtain ⟨n, rfl⟩ := hG g,
  rw [add_monoid_hom.map_gsmul, ←hm, ←gsmul_mul', ←gsmul_mul', mul_comm],
end

attribute [to_additive monoid_hom.map_add_cyclic] monoid_hom.map_cyclic

@[to_additive is_add_cyclic_of_order_of_eq_card] lemma is_cyclic_of_order_of_eq_card 
  [fintype α] (x : α) (hx : order_of x = fintype.card α) : is_cyclic α :=
begin
  classical,
  use x,
...
variables (β : Type*)
/-- **Order of an Add Subgroup** -/
lemma card_add_subgroup_dvd_card [add_group β] [fintype β] (s : add_subgroup β) [fintype s] :
  fintype.card s ∣ fintype.card β :=
by haveI := classical.prop_decidable; simp [add_subgroup.card_eq_card_quotient_add_card_add_subgroup s]
...
/-- A finite group of prime order is cyclic. -/
lemma is_add_cyclic_of_prime_card {α : Type u} [add_group α] [fintype α] {p : ℕ} [hp : fact p.prime]
  (h : fintype.card α = p) : is_add_cyclic α :=
⟨begin
  obtain ⟨g, hg⟩ : ∃ g : α, g ≠ 0,
  from fintype.exists_ne_of_one_lt_card (by { rw h, exact hp.1.one_lt }) 0,
  classical, -- for fintype (subgroup.gpowers g)
  have : fintype.card (add_subgroup.gmultiples g) ∣ p,
  { rw ←h,
    apply card_add_subgroup_dvd_card },
  rw nat.dvd_prime hp.1 at this,
  cases this,
  { rw fintype.card_eq_one_iff at this,
    cases this with t ht,
    suffices : g = 0,
    { contradiction },
    have hgt := ht ⟨g, by { change g ∈ add_subgroup.gmultiples g, exact add_subgroup.mem_gmultiples g }⟩,
    rw [←ht 0] at hgt,
    change (⟨_, _⟩ : add_subgroup.gmultiples g) = ⟨_, _⟩ at hgt,
    simpa using hgt },
  { use g,
    intro x,
    rw [←h] at this,
    rw add_subgroup.eq_top_of_card_eq _ this,
    exact add_subgroup.mem_top _ }
end⟩

attribute [to_additive is_add_cyclic_of_prime_card] is_cyclic_of_prime_card

@[to_additive order_of_eq_card_of_forall_mem_gmultiples_add] lemma order_of_eq_card_of_forall_mem_gpowers [fintype α]
...
@[to_additive bot.is_add_cyclic] instance bot.is_cyclic {α : Type u} [group α] : is_cyclic (⊥ : subgroup α) :=
...
-- not quite done
instance subgroup.is_add_cyclic {α : Type u} [add_group α] [is_add_cyclic α] (H : add_subgroup α) : is_add_cyclic H :=
by haveI := classical.prop_decidable; exact
let ⟨g, hg⟩ := is_add_cyclic.exists_generator α in
if hx : ∃ (x : α), x ∈ H ∧ x ≠ (0 : α) then
  let ⟨x, hx₁, hx₂⟩ := hx in
  let ⟨k, hk⟩ := hg x in
  have hex : ∃ n : ℕ, 0 < n ∧ n • g ∈ H,
    from ⟨k.nat_abs, nat.pos_of_ne_zero
      (λ h, hx₂ $ by {rw [← hk, int.eq_zero_of_nat_abs_eq_zero h], simp only, rw zero_gsmul}),
        match k, hk with
        | (k : ℕ), hk := begin rw [int.nat_abs_of_nat, ← gsmul_coe_nat], simp only at hk, rw hk; exact hx₁ end
        | -[1+ k], hk := begin rw int.nat_abs_of_neg_succ_of_nat, simp only at hk, rw ← add_subgroup.neg_mem_iff H, rw add_subgroup.neg_mem_iff; simp * at * end
        end⟩,
  ⟨⟨⟨g ^ nat.find hex, (nat.find_spec hex).2⟩,
    λ ⟨x, hx⟩, let ⟨k, hk⟩ := hg x in
      have hk₁ : g ^ ((nat.find hex : ℤ) * (k / nat.find hex)) ∈ gpowers (g ^ nat.find hex),
